...
void obj_free_half_list(obj_list *listo)
{
...
int obj_parse_mtl_file(char *filename, obj_list *material_list)
{
	int line_number = 0;
	char *current_token;
...
/*		
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		else if( strequal(current_token, "mtllib") ) // mtllib
		{
            ...
            obj_parse_mtl_file(strncat(strncat(dirname(filename), "/", 500),
                                      growable_data->material_filename, 500),
